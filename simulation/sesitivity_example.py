from core.models import Option, Outcome, OptionEvaluation, Criterion
from core.strategies import ExpectedValueStrategy, RiskAverseStrategy
from core.sensitivity import sensitivity_analysis

# Defining Options
options = [
    OptionEvaluation(
        option=Option("Job A"),
        outcomes={"salary": Outcome(120,100,80), "growth": Outcome(10,7,5), "risk": Outcome(5,7,10)}
    ),
    OptionEvaluation(
        option=Option("Job B"),
        outcomes={"salary": Outcome(110,95,80), "growth": Outcome(12,8,4), "risk": Outcome(3,5,8)}
    ),
]

# Defining Criteria
criteria = [
    Criterion("salary", 0.5, True),
    Criterion("growth", 0.3, True),
    Criterion("risk", 0.2, False)
]

# Choosing Strategy
strategy = RiskAverseStrategy()

#   Analyze sensitivity on risk_weight
results = sensitivity_analysis(options, criteria, strategy, risk_weights=[0.0, 0.25, 0.5, 0.75, 1.0])

for rw, scores in results.items():
    print(f"\n--- Risk Weight: {rw} ---")
    ranked = sorted(scores.items(), key=lambda x: x[1], reverse=True)
    for name, score in ranked:
        print(f"{name}: {score:.3f}")
